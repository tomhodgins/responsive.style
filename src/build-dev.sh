...
for file in *.md */*.md
  ...
  pandoc $file -f markdown -t html5 -H templates/header-dev.html -B templates/nav.html -A templates/footer-dev.html -o (echo "../$file" | sed '$s/\.md$/.html/') -s --data-dir=./ --highlight-style breezedark --variable=file:(echo "$file" | sed '$s/\.md$/.html/')
end
