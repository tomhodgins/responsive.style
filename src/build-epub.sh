...
for file in *.md */*.md
  echo "Converting "(echo "../$file" | sed '$s/\.md$/.epub/')
  pandoc $file -f markdown -t epub3 -H template/header-dev.html -B template/nav.html -A template/footer-dev.html -o (echo "../$file" | sed '$s/\.md$/.epub/') -s
end
